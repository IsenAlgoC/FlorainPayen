...
void supprimer_un_contact_dans_rep(Repertoire* rep, int indice) {
	...
	{						/* et que l'indice pointe a l'interieur */
		...
		if (indice < rep->nb_elts) {             // si l'indice est dans le tableau
			for (int i = indice; i < rep->nb_elts; i++) {   //on décale chaque element de la liste à partir de celui qu'on veut supprimer vers la gauche
				*(rep->tab + i) = *(rep->tab + i + 1);     // on va supprimer la velur a l'indice souhaité
			}
		}
...
void affichage_enreg_frmt(Enregistrement enr)
{
	...
	printf("\n| %-30s|%-30s|%-20s", enr.nom, enr.prenom, enr.tel); // permet de forcer l'aafichage sur 30 caractères, collé à gauche avec le -
...
{                                    // strcmp permet de comparer deux chaînes de caractères et de savoir si la première est inférieure,
	...
	if (strcmp(enr1.nom, enr2.nom) > 0) {
		...
		if (strcmp(enr1.prenom, enr2.prenom) > 0) { return(true); } // si on arrive là, les noms sont égaux, on recommence donc avec les prénoms
