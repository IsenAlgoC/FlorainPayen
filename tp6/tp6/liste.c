...
int InsertElementAt(LinkedList* Liste, int i, Enregistrement pers) {
	...
	if (CurrentElement != NULL) {
		...
		if (Liste->size == 0) {              // si la taille de la liste est nulle
			NewElement = NewLinkedListElement(pers); //on insere un nouvelle element
			if (NewElement != NULL) {        //on verifie que notre element n'est pas nul
		...
		if (Liste->size <= i) {               // si la taille de la liste est inferieur a l'indice de l'argument
			NewElement = NewLinkedListElement(pers);//on insere un nouvelle element
			if (NewElement != NULL) {        //on verifie que notre element n'est pas nul
